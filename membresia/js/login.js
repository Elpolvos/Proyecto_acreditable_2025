document.addEventListener('DOMContentLoaded', () => {
    const adminLoginBtn = document.getElementById('admin-login');
    const employeeLoginBtn = document.getElementById('employee-login');
    const userLoginBtn = document.getElementById('user-login');
    const loginForm = document.getElementById('login-form');
    const messageElement = document.getElementById('message');

    adminLoginBtn.addEventListener('click', () => {
        // Credenciales de administrador codificadas para la demostración
        const adminUser = {
            nombre: 'Admin',
            email: 'admin@example.com',
            role: 'admin'
        };
        sessionStorage.setItem('currentUser', JSON.stringify(adminUser));
        window.location.href = 'admin.html';
    });

    employeeLoginBtn.addEventListener('click', () => {
        // Credenciales de empleado codificadas para la demostración
        const employeeUser = {
            nombre: 'Empleado',
            email: 'employee@example.com',
            role: 'empleado'
        };
        sessionStorage.setItem('currentUser', JSON.stringify(employeeUser));
        window.location.href = 'employee.html';
    });

    userLoginBtn.addEventListener('click', () => {
        loginForm.classList.remove('hidden');
        adminLoginBtn.classList.add('hidden');
        employeeLoginBtn.classList.add('hidden');
        userLoginBtn.classList.add('hidden');
    });

    loginForm.addEventListener('submit', (e) => {
        e.preventDefault();

        const email = document.getElementById('email').value;
        const password = document.getElementById('password').value;

        const users = JSON.parse(localStorage.getItem('users')) || [];
        const user = users.find(user => user.email === email);

        if (user) {
            if (user.password === password) {
                messageElement.textContent = 'Inicio de sesión exitoso.';
                messageElement.style.color = 'green';

                user.role = 'usuario';
                sessionStorage.setItem('currentUser', JSON.stringify(user));

                setTimeout(() => {
                    window.location.href = 'dashboard.html';
                }, 2000);
            } else {
                messageElement.textContent = 'Contraseña incorrecta.';
                messageElement.style.color = 'red';
            }
        } else {
            messageElement.textContent = 'Necesita registrarse para poder iniciar sesión.';
            messageElement.style.color = 'red';
        }
    });
});
